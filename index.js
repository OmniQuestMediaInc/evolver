...
function rejectPendingRun(statePath) {
  try {
    ...
    console.warn(
      '[Loop] Failed to clear pending run state: ' + (e.message || e)
    );
  }
...
class CycleTimeoutError extends Error {
  constructor(timeoutMs, phase, cycleNum) {
    super(
      'Cycle hard-timeout exceeded after ' +
        timeoutMs +
        'ms (cycle=' +
        cycleNum +
        ', phase=' +
...
function spawnReplacementProcess({ reason, args, logPath }) {
  ...
  const allowOnWindows = parseBoolEnv(
    process.env.EVOLVER_SUICIDE_WINDOWS,
    false
  );
  if (isWindows && !allowOnWindows) {
    console.log(
      '[Daemon] Skipping in-process respawn on Windows (' +
        reason +
        '). ' +
  ...
  try {
    ...
    console.error(
      '[Daemon] Spawn-replacement failed (' +
        reason +
        '): ' +
        ((e && e.message) || e)
    );
...
async function main() {
  ...
  if (!command || command === 'run' || command === '/evolve' || isLoop) {
    ...
    try {
      ...
      console.error('');
      console.error(
        '[Preflight] Could not run "git --version". Evolver requires git to be installed and available on PATH.'
      );
      console.error(
        '[Preflight] On Windows: install Git from https://git-scm.com/download/win and make sure `git --version` works in a fresh terminal.'
      );
      console.error(
        '[Preflight] On macOS:   xcode-select --install  (or `brew install git`)'
      );
      console.error(
        '[Preflight] On Linux:   sudo apt-get install -y git  (or your distro equivalent)'
      );
      console.error('');
      process.exit(1);
    }
    ...
    if (isLoop) {
      ...
      function shutdown() {
        releaseLock();
        try {
          require('./src/gep/a2aProtocol').stopEventStream();
        } catch (e) {}
      }
      ...
      process.on('unhandledRejection', reason => {
        ...
        if (_unhandledRejectionCount >= 5) {
          console.error(
            '[FATAL] Too many unhandled rejections (' +
              _unhandledRejectionCount +
              '). Exiting to avoid corrupt state.'
          );
          releaseLock();
          process.exit(1);
        }
      ...
      try {
        ...
        const hasAnySource =
          diag.agentSessionsDirExists ||
          diag.cursorDirExists ||
          diag.claudeDirExists ||
          diag.codexDirExists ||
          Boolean(diag.cursorTranscriptsDir);
        if (!hasAnySource) {
          console.warn(
            '[Daemon] No session sources detected at startup. Loop mode runs background self-maintenance but cannot observe a live agent without at least one of:'
          );
          console.warn(
            `  - ~/.openclaw/agents/<AGENT_NAME>/sessions/ (current AGENT_NAME=${diag.agentName}, exists=${diag.agentSessionsDirExists})`
          );
          console.warn(
            '  - ~/.cursor / ~/.claude / ~/.codex (IDE transcripts)'
          );
          console.warn(
            '  - EVOLVER_CURSOR_TRANSCRIPTS_DIR (explicit override)'
          );
          if (diag.availableOpenClawAgents.length > 0) {
            console.warn(
              `  Available OpenClaw agents under ~/.openclaw/agents/: ${diag.availableOpenClawAgents.join(', ')}`
            );
            console.warn(
              '  Set AGENT_NAME=<agent> or AGENT_SESSIONS_DIR=<abs path> to the one actually doing work.'
            );
          }
          ...
          console.warn(
            '  If you want real-time agent assist (not background self-maintenance), run `evolver run` from inside the agent session instead of `evolver --loop`.'
          );
        }
      } catch (_diagErr) {
      ...
      try {
        ...
        const hubUrl =
          typeof a2a.getHubUrl === 'function' ? a2a.getHubUrl() : '';
        const nodeId =
          typeof a2a.getNodeId === 'function' ? a2a.getNodeId() : '';
        const hasSecret =
          typeof a2a.getHubNodeSecret === 'function' &&
          !!a2a.getHubNodeSecret();
        if (mirrorOff) {
          console.log(
            '[HubMirror] DISABLED — set MEMORY_GRAPH_SYNC_HUB=1 (or unset it) to mirror outcome events to <hub>/a2a/memory/event.'
          );
        } else if (!hubUrl || !nodeId || !hasSecret) {
          console.log(
            `[HubMirror] inactive — missing one of: hub=${hubUrl ? 'OK' : 'MISSING'} node_id=${nodeId ? 'OK' : 'MISSING'} secret=${hasSecret ? 'OK' : 'MISSING'}. Local memory graph is unaffected.`
          );
        } else {
          console.log(
            `[HubMirror] ENABLED — outcome/attempt/solidify/skill_emit events mirror to ${hubUrl}/a2a/memory/event as ${nodeId}. Set MEMORY_GRAPH_SYNC_HUB=0 to disable.`
          );
        }
      } catch (_mirrorDiagErr) {
        /* diagnostics must never block startup */
      }
      ...
      try {
        ...
        const sampleRateRaw = Number(
          process.env.EVOLVE_RECALL_VERIFY_SAMPLE_RATE
        );
        const sampleRate =
          Number.isFinite(sampleRateRaw) &&
          sampleRateRaw >= 0 &&
          sampleRateRaw <= 1
            ? sampleRateRaw
            : 1.0;
        if (!enabled) {
          console.log(
            '[RecallVerify] DISABLED — set EVOLVE_RECALL_VERIFY=1 to verify published assets round-trip via Hub Phase 2 lookup.'
          );
        } else {
          console.log(
            `[RecallVerify] ENABLED — verifying published assets via Hub Phase 2 lookup, sample_rate=${sampleRate}. Set EVOLVE_RECALL_VERIFY=0 to disable.`
          );
        }
      } catch (_rvDiagErr) {
        /* diagnostics must never block startup */
      ...
      const cycleProgressPath = path.join(
        getEvolutionDir(),
        'cycle_progress.json'
      );
      ...
      const suicideEnabled =
        String(process.env.EVOLVER_SUICIDE || '').toLowerCase() !== 'false';
      ...
      const cycleTimeoutEnabled = parseBoolEnv(
        process.env.EVOLVER_CYCLE_TIMEOUT_ENABLED,
        true
      );
      const cycleTimeoutMs = parseMs(
        process.env.EVOLVER_CYCLE_TIMEOUT_MS,
        2700000
      ); // 45 min default
      const progressUpdateMs = parseMs(
        process.env.EVOLVER_PROGRESS_UPDATE_MS,
        60000
      ); // 1 min default
      ...
      try {
        if (
          ...
          } = require('./src/gep/mailboxTransport');
          registerMailboxTransport();
          process.env.A2A_TRANSPORT = 'mailbox';
        } else {
          const a2a = require('./src/gep/a2aProtocol');
          try {
            ...
            console.warn(
              '[Heartbeat] startHeartbeat failed: ' +
                ((hbErr && hbErr.message) || hbErr)
            );
          }
          try {
            ...
            console.warn(
              '[SSE] startEventStream failed: ' +
                ((ssErr && ssErr.message) || ssErr)
            );
          }
        }
      } catch (e) {
      ...
      try {
        ...
        console.warn(
          '[RecallVerify] startWorker failed: ' +
            ((rvStartErr && rvStartErr.message) || rvStartErr)
        );
      }
      ...
      try {
        ...
        console.warn(
          '[ValidatorDaemon] failed to start: ' +
            ((vdErr && vdErr.message) || vdErr)
        );
      }
      ...
      try {
        ...
        if (atpMode === 'auto' || atpMode === 'on') {
          const hubUrl =
            process.env.A2A_HUB_URL || process.env.EVOMAP_HUB_URL || '';
          if (hubUrl) {
            ...
            merchantAgent
              ...
              .catch(function (atpErr) {
                console.warn(
                  '[ATP] merchantAgent.start failed: ' +
                    ((atpErr && atpErr.message) || atpErr)
                );
              });
          }
        }
        ...
        console.warn(
          '[ATP] Auto-init failed: ' +
            ((atpInitErr && atpInitErr.message) || atpInitErr)
        );
      }
      ...
      try {
        try {
          ...
          console.warn(
            '[ATP-AutoBuyer] first-run prompt failed: ' +
              ((promptErr && promptErr.message) || promptErr)
          );
        }
        const autoBuyRaw = (process.env.EVOLVER_ATP_AUTOBUY || 'on')
          .toLowerCase()
          .trim();
        const autoBuyOn =
          autoBuyRaw !== 'off' && autoBuyRaw !== '0' && autoBuyRaw !== 'false';
        if (autoBuyOn) {
          const hubUrl =
            process.env.A2A_HUB_URL || process.env.EVOMAP_HUB_URL || '';
          if (hubUrl) {
            ...
            autoBuyer.start({
              dailyCap:
                Number(process.env.ATP_AUTOBUY_DAILY_CAP_CREDITS) || undefined,
              perOrderCap:
                Number(process.env.ATP_AUTOBUY_PER_ORDER_CAP_CREDITS) ||
                undefined,
            });
          } else {
            console.warn(
              '[ATP-AutoBuyer] autobuy enabled but no hub URL configured, skipping.'
            );
          }
        }
        const autoDeliverRaw = (process.env.EVOLVER_ATP_AUTODELIVER || 'on')
          .toLowerCase()
          .trim();
        const autoDeliverOn =
          autoDeliverRaw !== 'off' &&
          autoDeliverRaw !== '0' &&
          autoDeliverRaw !== 'false';
        if (autoDeliverOn) {
          const hubUrl =
            process.env.A2A_HUB_URL || process.env.EVOMAP_HUB_URL || '';
          if (hubUrl) {
            ...
            console.warn(
              '[ATP-AutoDeliver] autodeliver enabled but no hub URL configured, skipping.'
            );
          }
        }
      } catch (autoBuyInitErr) {
        console.warn(
          '[ATP-AutoBuyer] Init failed: ' +
            ((autoBuyInitErr && autoBuyInitErr.message) || autoBuyInitErr)
        );
      }
      ...
      while (true) {
        try {
          ...
          if (progressUpdateMs > 0) {
            ...
            if (typeof progressTicker.unref === 'function')
          ...
          try {
            ...
            if (cycleTimeoutEnabled && cycleTimeoutMs > 0) {
              const timeoutPromise = new Promise(function (_, reject) {
                ...
                if (
                  cycleTimeoutHandle &&
                  typeof cycleTimeoutHandle.unref === 'function'
                )
                  cycleTimeoutHandle.unref();
              });
          ...
          try {
                ...
                try {
                  if (shouldDF()) {
                    ...
                    if (dfResult && dfResult.ok) {
                      console.log(
                        '[OMLS] Idle-window failure distillation: ' +
                          dfResult.gene.id
                      );
                    }
                  }
                  ...
                  if (isVerbose)
                    console.warn('[OMLS] Distill error: ' + (e.message || e));
                }
              }
              if (schedule.should_explore) {
                try {
                  const exploreResult = await tryExplore(
                    [],
                    schedule,
                    getRepoRoot()
                  );
                  if (
                    exploreResult &&
                    exploreResult.signals &&
                    exploreResult.signals.length > 0
                  ) {
                    console.log(
                      '[OMLS] Explore discovered ' +
                        exploreResult.signals.length +
                        ' signals: ' +
                        exploreResult.signals.slice(0, 5).join(', ')
                    );
                  }
                } catch (e) {
                  if (isVerbose)
                    console.warn('[OMLS] Explore error: ' + (e.message || e));
                }
              }
            ...
            if (isVerbose)
              console.warn('[OMLS] Scheduler error: ' + (e.message || e));
          }
          ...
          if (suicideEnabled) {
            ...
            if (cycleCount >= maxCyclesPerProcess || memMb > maxRssMb) {
              ...
              if (result.spawned) {
                ...
                console.log(
                  '[Daemon] Exiting with code 1 to let external supervisor respawn.'
                );
                releaseLock();
                process.exit(1);
              } else {
          ...
          try {
            ...
            if (lastSignals.includes('force_steady_state')) {
              ...
              console.log(
                '[Daemon] Saturation detected. Entering steady-state mode (4x sleep).'
              );
            } else if (lastSignals.includes('evolution_saturation')) {
              saturationMultiplier = 2;
              console.log(
                '[Daemon] Approaching saturation. Reducing evolution frequency (2x sleep).'
              );
            }
          } catch (e) {
            if (isVerbose)
              console.warn(
                '[Daemon] Saturation check error: ' + (e.message || e)
              );
          }
          ...
          if (isVerbose) {
            ...
            console.log(
              `[Verbose] cycle=${cycleCount} ok=${ok} dt=${dt}ms sleep=${totalSleepMs}ms (base=${currentSleepMs} jitter=${jitter} sat=${saturationMultiplier}x) rss=${memMb}MB signals=[${signals}]`
            );
          }
          writeCycleProgressAtomic(cycleProgressPath, {
            pid: process.pid,
        ...
        try {
          ...
          if (shouldDistillFromFailures()) {
            ...
            if (failureResult && failureResult.ok && failureResult.gene) {
              console.log(
                '[Distiller] Repair gene distilled from failures: ' +
                  failureResult.gene.id
              );
            }
          }
        } catch (e) {
          console.warn(
            '[Distiller] Init failed (non-fatal): ' + (e.message || e)
          );
        }
      }
          ...
          if (!res || !res.ok) {
            if (res && res.validation && !res.validation.ok) {
              ...
              const failedStep =
                ...
                res.validation.results.find(function (r) {
                  return !r.ok;
                });
              urgentOpts.validationErrors = failedStep
                ? failedStep.err || failedStep.cmd || ''
                : '';
            }
            ...
            if (
              evtOutcome &&
              typeof evtOutcome.score === 'number' &&
              evtOutcome.score < 0.3
            ) {
            ...
            if (
              ...
              urgentOpts.signals =
                res.event && Array.isArray(res.event.signals)
                  ? res.event.signals
                  : [];
            }
            if (
              res &&
              res.constraintCheck &&
              Array.isArray(res.constraintCheck.violations)
            ) {
              const llmRejectV = res.constraintCheck.violations.find(
                function (v) {
                  return String(v).startsWith('llm_review_rejected');
                }
              );
              if (llmRejectV) {
                urgentOpts.llmReviewRejected = true;
                urgentOpts.llmReviewReason = String(llmRejectV).replace(
                  'llm_review_rejected: ',
                  ''
                );
              }
            }
            const lr = readJsonSafe(
              path.join(
                require('./src/gep/paths').getEvolutionDir(),
                'evolution_solidify_state.json'
              )
            );
            if (lr && lr.last_run && lr.last_run.active_task_id) {
              ...
              urgentOpts.taskSignals = Array.isArray(lr.last_run.task_signals)
                ? lr.last_run.task_signals.join(', ')
                : '';
            }
          } else if (
            res.event &&
            res.event.outcome &&
            res.event.outcome.score < 0.3
          ) {
            urgentOpts.lowConfidence = true;
          ...
          if (Object.keys(urgentOpts).length > 0) {
            ...
            if (urgentQs.length > 0) {
              console.log(
                '[UrgentQ] Generated ' +
                  urgentQs.length +
                  ' urgent question(s) from solidify outcome.'
              );
              try {
                const fetchRes = await fetchTasks({ questions: urgentQs });
                if (fetchRes.questions_created) {
                  const accepted = fetchRes.questions_created.filter(
                    function (q) {
                      return !q.error;
                    }
                  );
                  if (accepted.length > 0) {
                    console.log(
                      '[UrgentQ] Hub accepted ' +
                        accepted.length +
                        ' urgent question(s) as bounties.'
                    );
                  }
                }
              } catch (err) {
                console.log(
                  '[UrgentQ] Send failed (non-fatal): ' +
    ...
    const responseFilePath = responseFileFlag.slice('--response-file='.length);
    {
      ...
      if (
        responseFilePath.includes('..') ||
        !resolvedResponsePath.startsWith(resolvedRepoRoot)
      ) {
        console.error(
          '[Distill] ERROR: Invalid response-file path "' +
            responseFilePath +
            '" - path traversal detected or path is outside the repository.'
        );
        process.exit(2);
      }
    }
    ...
    if (gene) {
      ...
      console.log(
        '  (no gene selected or gene not found: ' + (geneId || 'none') + ')'
      );
    }
    ...
    if (args.includes('--approve')) {
      ...
      try {
        ...
        execSync(
          'git clean -fd -e node_modules -e workspace -e .env -e ".env.*" -e "*.pid"',
          {
            cwd: repoRoot,
            encoding: 'utf8',
            timeout: 30000,
    ...
    const eqFlag = args.find(
      a =>
        typeof a === 'string' &&
        (a.startsWith('--skill=') || a.startsWith('-s='))
    );
    if (eqFlag) {
      ...
      if (
        flagIdx !== -1 &&
        args[flagIdx + 1] &&
        !String(args[flagIdx + 1]).startsWith('-')
      ) {
    ...
    if (!skillId) {
      ...
      if (positional && !String(positional).startsWith('-'))
        skillId = positional;
    }
    ...
    try {
      if (!getHubNodeSecret()) {
        console.log(
          '[fetch] No node_secret found. Sending hello to Hub to register...'
        );
      ...
      if (!resp.ok) {
        ...
        if (resp.status === 404) {
          console.error(
            '  Skill "' + skillId + '" not found or not publicly available.'
          );
          console.error(
          ...
          console.error(
            '    2. Re-register: set A2A_NODE_ID and run fetch again'
          );
        } else if (resp.status === 402) {
          console.error(
            '  Insufficient credits. Check your balance at https://evomap.ai'
          );
        } else if (resp.status >= 500) {
          console.error(
            '  Server error. The Hub may be temporarily unavailable.'
          );
          console.error(
            '  Try again in a few minutes. If the issue persists, report at:'
          );
        ...
        if (isVerbose) {
          ...
          console.error(
            '[Verbose] Status: ' + resp.status + ' ' + (resp.statusText || '')
          );
          console.error(
            '[Verbose] Response body: ' + (body || '(empty)').slice(0, 2000)
          );
        }
        process.exit(1);
      }
      ...
      const outFlag = args.find(
        a => typeof a === 'string' && a.startsWith('--out=')
      );
      const safeId = String(data.skill_id || skillId).replace(
        /[^a-zA-Z0-9_\-\.]/g,
        '_'
      );
      ...
      if (
        ...
        console.error(
          '[fetch] Hub returned an invalid skill_id: ' + JSON.stringify(safeId)
        );
        process.exit(1);
      }
      let outDir;
      if (outFlag) {
        ...
        if (rel.startsWith('..') || path.isAbsolute(rel)) {
          console.error(
            '[fetch] --out= must resolve to a path inside the current working directory'
          );
        ...
        if (rel.startsWith('..') || path.isAbsolute(rel)) {
          console.error(
            '[fetch] Hub-provided skill_id escapes skills/ directory: ' +
      ...
      const bundled = Array.isArray(data.bundled_files)
        ? data.bundled_files
        : [];
      const skippedFiles = [];
      for (const file of bundled) {
        ...
        if (!ALLOWED_SKILL_EXTENSIONS.has(ext)) {
          console.warn(
            '[fetch] Skipped skill file with disallowed extension: ' + safeName
          );
        ...
        if (Buffer.byteLength(file.content, 'utf8') > MAX_SKILL_FILE_BYTES) {
          console.warn(
            '[fetch] Skipped skill file exceeding ' +
        ...
        if (relToOut.startsWith('..') || path.isAbsolute(relToOut)) {
          console.warn(
            '[fetch] Skipped bundled file whose resolved path escapes outDir: ' +
              safeName
          );
          skippedFiles.push(safeName);
          continue;
        ...
        if (path.dirname(destPath) === resolvedCwd) {
          console.warn(
            '[fetch] Skipped bundled file that would land in cwd: ' + safeName
          );
          skippedFiles.push(safeName);
          continue;
        }
      ...
      if (error && error.name === 'TimeoutError') {
        console.error(
          '[fetch] Request timed out (30s). Check your network and A2A_HUB_URL.'
        );
    ...
    try {
      if (!getHubNodeSecret()) {
        console.log(
          '[sync] No node_secret found. Sending hello to Hub to register...'
        );
        const helloResult = await sendHelloToHub();
        if (!helloResult || !helloResult.ok) {
          console.error(
            '[sync] Failed to register with Hub:',
            (helloResult && helloResult.error) || 'unknown'
          );
          process.exit(1);
        }
      ...
      const typeFilter = (function () {
        const f = args.find(function (a) {
          return typeof a === 'string' && a.startsWith('--type=');
        });
        return f ? f.slice('--type='.length) : null;
      })();
      const scopeArg = (function () {
        const f = args.find(function (a) {
          return typeof a === 'string' && a.startsWith('--scope=');
        });
        return f ? f.slice('--scope='.length) : 'all';
      })();
      const statusFilter = (function () {
        const f = args.find(function (a) {
          return typeof a === 'string' && a.startsWith('--status=');
        });
        return f ? f.slice('--status='.length) : null;
      })();
      const exportPath = (function () {
        const f = args.find(function (a) {
          return typeof a === 'string' && a.startsWith('--export=');
        });
      ...
      if (!validScopes.has(scopeArg)) {
        console.error(
          '[sync] Invalid --scope=' +
            scopeArg +
            '. Expected: all, purchased, published.'
        );
        process.exit(1);
      }
      ...
      if (doPurchased) {
        ...
        console.log(
          '[sync]   purchased: ' + purchasedAssets.length + ' asset(s)'
        );
      }
      if (doPublished) {
        console.log(
          '[sync] Fetching published-by-me assets from Hub (includes drafts)...'
        );
        publishedAssets = await fetchAllPages('/a2a/assets/published-by-me', {
          status: statusFilter,
        });
        console.log(
          '[sync]   published: ' + publishedAssets.length + ' asset(s)'
        );
      }
          ...
          if (assetType === 'Gene' && localGeneIds.has(localId)) {
            if (isVerbose)
              console.warn(
                '  [sync] Skipping ' +
                  localId +
                  ' (local id collision; pass --force to overwrite with Hub copy)'
              );
            skippedIdCollision++;
            continue;
          }
        ...
        try {
          ...
          if (!payload) {
            ...
            if (!detailResp.ok) {
              if (isVerbose)
                console.warn(
                  '  [sync] Failed to fetch detail for ' +
                    assetId +
                    ' (HTTP ' +
                    detailResp.status +
                    ')'
          ...
          if (assetType === 'Gene') {
            ...
            const capsuleObj = {
              ...
              genes_used: Array.isArray(payload.genes_used)
                ? payload.genes_used
                : [],
      ...
      if (skippedIdCollision > 0 && !force) {
        console.log(
          '[sync] ' +
            skippedIdCollision +
            ' Hub asset(s) share a local id with an existing local entry that has no hub_asset_id.'
        );
        console.log(
          '[sync] Re-run with --force to overwrite those local entries with the Hub copies.'
        );
      }
      ...
      if (listUnpublished && doPublished) {
        ...
        if (unpublishedGenes.length || unpublishedCapsules.length) {
          console.log(
            '[sync] Local-only (not on Hub): genes=' +
              unpublishedGenes.length +
              ' capsules=' +
              unpublishedCapsules.length
          );
          if (isVerbose) {
            for (const g of unpublishedGenes.slice(0, 20))
              console.log('    gene: ' + g.id);
            for (const c of unpublishedCapsules.slice(0, 20))
              console.log('    capsule: ' + c.id);
            if (unpublishedGenes.length + unpublishedCapsules.length > 40) {
              console.log(
                '    ... (truncated; use --export=<path>.gepx to bundle all)'
              );
            }
          }
        }
      ...
      if (exportPath) {
        if (dryRun) {
          ...
          const memoryGraphPath = require('path').join(
            getMemoryDir(),
            'memory_graph.jsonl'
          );
          try {
            ...
            console.log(
              '[sync]   stats: ' + JSON.stringify(result.manifest.statistics)
            );
          } catch (exportErr) {
            console.error(
              '[sync] Export failed: ' +
                ((exportErr && exportErr.message) || exportErr)
            );
            process.exit(1);
          }
        }
      ...
      if (error && error.name === 'TimeoutError') {
        console.error(
          '[sync] Request timed out. Check your network and A2A_HUB_URL.'
        );
      } else {
    ...
    const portFlag = args.find(
      a => typeof a === 'string' && a.startsWith('--port=')
    );
    const port = portFlag
      ? Number(portFlag.slice('--port='.length))
      : undefined;
    ...
    const { setupHooks, resolveConfigRoot, detectPlatform, loadAdapter } =
      hookAdapter;

    const platformFlag = args.find(
      a => typeof a === 'string' && a.startsWith('--platform=')
    );
    const platform = platformFlag
      ? platformFlag.slice('--platform='.length)
      : undefined;
    const force = args.includes('--force');
    ...
    if (verifyOnly) {
      ...
      try {
        ...
        if (!platformId) {
          console.error(
            '[setup-hooks] --verify: could not detect platform. Pass --platform=opencode|cursor|claude-code|codex|kiro'
          );
          process.exit(2);
        }
        ...
        if (!adapter || typeof adapter.verify !== 'function') {
          console.error(
            '[setup-hooks] --verify: platform ' +
              platformId +
              ' does not support verification yet.'
          );
          process.exit(2);
        }
    ...
    try {
      ...
      if (result && result.ok) {
        ...
        console.error(
          '[setup-hooks] Failed: ' + ((result && result.error) || 'unknown')
        );
        process.exit(1);
      }
    } catch (error) {
    ...
    try {
      ...
      function flag(name) {
        const pref = '--' + name + '=';
        const hit = subArgs.find(function (a) {
          return typeof a === 'string' && a.startsWith(pref);
        });
      ...
      if (!taskId || !orderId || !answerFile) {
        console.error(
          '[ATP-Complete] Missing required flags: --task-id, --order-id, --answer-file'
        );
        console.error(
          'Usage: node index.js atp-complete --task-id=<tid> --order-id=<oid> --answer-file=<path> [--summary="..."] [--capabilities=cap1,cap2] [--signals=sig1,sig2]'
        );
        process.exit(2);
      }
      ...
      if (res && res.ok) {
        console.log(
          '[ATP-Complete] OK asset_id=' +
            res.assetId +
            (res.deliveryId ? ' delivery_id=' + res.deliveryId : '')
        );
        process.exit(0);
      }
      console.error(
        '[ATP-Complete] FAILED stage=' +
          (res && res.stage) +
          ' error=' +
          (res && res.error)
      );
      ...
      console.error(
        '[ATP-Complete] Error:',
        (atpCompleteErr && atpCompleteErr.message) || atpCompleteErr
      );
      process.exit(1);
    }
  } else if (
    ...
    try {
      ...
      console.error(
        '[ATP] CLI error:',
        (atpCliErr && atpCliErr.message) || atpCliErr
      );
      process.exit(1);
    }
  } else {
