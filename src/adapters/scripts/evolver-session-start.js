...
function formatOutcome(entry) {
  ...
  const score =
    entry.outcome && entry.outcome.score != null ? entry.outcome.score : '?';
  ...
  return `[${icon}] ${ts} score=${score} signals=[${signals}] ${note}`.slice(
    0,
    200
  );
}
...
function shouldSkipInjection() {
  ...
  const dedupEnabled =
    String(process.env.EVOLVER_SESSION_START_DEDUP || '').toLowerCase() ===
      '1' ||
    String(process.env.EVOLVER_SESSION_START_DEDUP || '').toLowerCase() ===
      'true';
  ...
  const ttlMs =
    Number(process.env.EVOLVER_SESSION_START_DEDUP_TTL_MS) || 30 * 60 * 1000;
...
function main() {
  ...
  const successCount = entries.filter(
    e => e.outcome && e.outcome.status === 'success'
  ).length;
  const failCount = entries.filter(
    e => e.outcome && e.outcome.status === 'failed'
  ).length;
