...
const HEARTBEAT_FIRST_DELAY_MS = envInt(
  'EVOLVER_HEARTBEAT_FIRST_DELAY_MS',
  30000
);
const EVENT_POLL_TIMEOUT_MS = envInt('EVOLVER_EVENT_POLL_TIMEOUT_MS', 60000);
const HTTP_TRANSPORT_TIMEOUT_MS = envInt(
  'EVOLVER_HTTP_TRANSPORT_TIMEOUT_MS',
  15000
);
...
const SOLIDIFY_RETRY_INTERVAL_MS = envInt(
  'EVOLVER_SOLIDIFY_RETRY_INTERVAL_MS',
  1000
);
...
const GENE_BAN_PER_KEY_ATTEMPTS = envInt(
  'EVOLVER_GENE_BAN_PER_KEY_ATTEMPTS',
  4
);
const GENE_BAN_BEST_THRESHOLD = envFloat(
  'EVOLVER_GENE_BAN_BEST_THRESHOLD',
  0.15
);
const GENE_EPIGENETIC_HARD_BOOST = envFloat(
  'EVOLVER_GENE_EPIGENETIC_HARD_BOOST',
  -0.3
);
...
const MEMORY_FRAGMENT_MAX_CHARS = envInt(
  'EVOLVER_MEMORY_FRAGMENT_MAX_CHARS',
  50000
);
...
const CLEANUP_MAX_AGE_MS = envInt(
  'EVOLVER_CLEANUP_MAX_AGE_MS',
  24 * 60 * 60 * 1000
);
...
const SELF_PR_COOLDOWN_MS = envInt(
  'EVOLVER_SELF_PR_COOLDOWN_MS',
  24 * 60 * 60 * 1000
);
...
const VALIDATOR_ENABLED = (function () {
  const v = String(process.env.EVOLVER_VALIDATOR_ENABLED || '')
    .toLowerCase()
    .trim();
  return v === '1' || v === 'true' || v === 'yes' || v === 'on';
})();
const VALIDATOR_STAKE_AMOUNT = envInt('EVOLVER_VALIDATOR_STAKE_AMOUNT', 100);
const VALIDATOR_MAX_TASKS_PER_CYCLE = envInt(
  'EVOLVER_VALIDATOR_MAX_TASKS_PER_CYCLE',
  2
);
const VALIDATOR_FETCH_TIMEOUT_MS = envInt(
  'EVOLVER_VALIDATOR_FETCH_TIMEOUT_MS',
  8000
);
const VALIDATOR_REPORT_TIMEOUT_MS = envInt(
  'EVOLVER_VALIDATOR_REPORT_TIMEOUT_MS',
  10000
);
const VALIDATOR_STAKE_TIMEOUT_MS = envInt(
  'EVOLVER_VALIDATOR_STAKE_TIMEOUT_MS',
  10000
);
const VALIDATOR_CMD_TIMEOUT_MS = envInt(
  'EVOLVER_VALIDATOR_CMD_TIMEOUT_MS',
  60000
);
const VALIDATOR_BATCH_TIMEOUT_MS = envInt(
  'EVOLVER_VALIDATOR_BATCH_TIMEOUT_MS',
  180000
);
