...
function getAssetOverview(query = {}) {
  ...
  const events = readJsonl(paths.eventsPath, {
    last: query.eventsLast || 500,
  }).map(redactValue);
  const candidates = readJsonl(paths.candidatesPath, {
    last: query.candidatesLast || 200,
  }).map(redactValue);
  const externalCandidates = readJsonl(paths.externalCandidatesPath, {
    last: query.candidatesLast || 200,
  }).map(redactValue);
  const assetCalls = readJsonl(paths.assetCallLogPath, {
    last: query.assetCallsLast || 500,
  }).map(redactValue);
  ...
  return {
    ...
    capsulesByOutcome: countBy(
      capsules,
      c => (c.outcome && c.outcome.status) || 'unknown'
    ),
...
function listCapsules(query = {}) {
  const paths = getObserverPaths();
  const all = readCapsules(paths).concat(
    readFailedCapsules(paths).map(c => ({ ...c, failed_store: true }))
  );
  return {
...
function listEvents(query = {}) {
  const events = readJsonl(getObserverPaths().eventsPath)
    .map(redactValue)
    .reverse();
...
function listCandidates(query = {}) {
  const paths = getObserverPaths();
  const local = readJsonl(paths.candidatesPath).map(entry => ({
    ...entry,
    source: 'local',
  }));
  const external = readJsonl(paths.externalCandidatesPath).map(entry => ({
    ...entry,
    source: 'external',
  }));
  return paginate(
    filterText(local.concat(external).map(redactValue).reverse(), query.q),
    query
  );
}

function listAssetCalls(query = {}) {
  const calls = readJsonl(getObserverPaths().assetCallLogPath)
    .map(redactValue)
    .reverse();
...
function readGenes(paths) {
  ...
  const fromJsonl = readJsonl(paths.genesJsonlPath).filter(
    g => g && g.type === 'Gene'
  );
...
function readCapsules(paths) {
  const fromJson =
    readJsonSafe(paths.capsulesPath, { capsules: [] }).capsules || [];
  const fromJsonl = readJsonl(paths.capsulesJsonlPath).filter(
    c => c && c.type === 'Capsule'
  );
  return dedupeById(fromJson.concat(fromJsonl)).map(redactValue);
}
...
function readFailedCapsules(paths) {
  const failed =
    readJsonSafe(paths.failedCapsulesPath, { failed_capsules: [] })
      .failed_capsules || [];
...
function countBy(items, keyOrFn) {
  ...
  for (const item of items) {
    const key =
      typeof keyOrFn === 'function' ? keyOrFn(item) : item && item[keyOrFn];
...
function matchesAsset(item, id) {
  ...
  return (
    item.id === id || item.asset_id === id || item.asset_id === `sha256:${id}`
  );
}
