...
async function runPrompt(opts) {
  ...
  try {
    output.write('\n');
    output.write(
      '[ATP-AutoBuyer] Your evolver can automatically place small-priced\n'
    );
    output.write('ATP orders when it detects a capability gap (default ON).\n');
    output.write(
      '  - daily hard cap:   ATP_AUTOBUY_DAILY_CAP_CREDITS (default applies)\n'
    );
    output.write('  - per-order cap:    ATP_AUTOBUY_PER_ORDER_CAP_CREDITS\n');
    output.write(
      '  - set EVOLVER_ATP_AUTOBUY=off and restart to disable at any time.\n'
    );
    output.write('\n');
  } catch (_) {
  ...
  try {
    answer = await ask(
      'Keep autoBuyer enabled for this session? [y/n/later] ',
      {
        input,
        output,
      }
