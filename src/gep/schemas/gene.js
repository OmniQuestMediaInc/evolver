...
function createGene(partial) {
  ...
  g.signals_match = Array.isArray(g.signals_match)
    ? g.signals_match.slice()
    : [];
  g.strategy = Array.isArray(g.strategy) ? g.strategy.slice() : [];
  g.validation = Array.isArray(g.validation) ? g.validation.slice() : [];
  g.preconditions = Array.isArray(g.preconditions)
    ? g.preconditions.slice()
    : [];
  g.epigenetic_marks = Array.isArray(g.epigenetic_marks)
    ? g.epigenetic_marks.slice()
    : [];
  g.learning_history = Array.isArray(g.learning_history)
    ? g.learning_history.slice()
    : [];
  g.anti_patterns = Array.isArray(g.anti_patterns)
    ? g.anti_patterns.slice()
    : [];
  ...
  if (
    !Array.isArray(g.constraints.forbidden_paths) ||
    g.constraints.forbidden_paths.length === 0
  ) {
  ...
  if (typeof g.schema_version !== 'string')
    g.schema_version = GENE_DEFAULTS.schema_version;
...
function validateGene(g) {
  ...
  if (g.type !== 'Gene')
    throw new Error('Gene.type must be "Gene", got: ' + g.type);
  if (!g.id || typeof g.id !== 'string')
    throw new Error('Gene.id is required and must be a string');
  ...
  if (!Array.isArray(g.signals_match))
    throw new Error('Gene.signals_match must be an array');
  if (!Array.isArray(g.strategy))
    throw new Error('Gene.strategy must be an array');
  return true;
}
