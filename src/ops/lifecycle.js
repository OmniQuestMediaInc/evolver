...
function listProcesses() {
  if (process.platform === 'win32') {
    var out = execText(
      "powershell -NoProfile -Command \"Get-CimInstance Win32_Process | ForEach-Object { $cmd = if ($_.CommandLine) { $_.CommandLine } else { '' }; Write-Output ('{0}\\t{1}' -f $_.ProcessId, $cmd) }\""
    );
    var procs = [];
    for (var line of out.split(/\r?\n/)) {
      ...
      var pidText =
        tabIndex >= 0 ? line.slice(0, tabIndex).trim() : line.trim();
...
function getRunningPids() {
  try {
    ...
    for (var proc of listProcesses()) {
      ...
      if (
        cmdLower.includes('node') &&
        cmdLower.includes('index.js') &&
        cmdLower.includes('--loop')
      ) {
        pids.push(pid);
      }
    }
...
function getCmdLine(pid) {
  try {
    ...
    var proc = listProcesses().find(function (p) {
      return p.pid === safePid;
    });
