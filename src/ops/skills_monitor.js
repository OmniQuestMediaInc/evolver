    ...
    try {
      ...
      if (pkg.dependencies && Object.keys(pkg.dependencies).length > 0) {
        if (!fs.existsSync(path.join(skillPath, 'node_modules'))) {
          ...
          try {
            if (
              fs.readdirSync(path.join(skillPath, 'node_modules')).length === 0
            ) {
  ...
  for (var i = 0; i < issues.length; i++) {
    if (
      ...
      try {
        ...
        execSync(
          'npm install --production --no-audit --no-fund --ignore-scripts',
          {
            cwd: skillPath,
            stdio: 'ignore',
            timeout: 60000, // Increased timeout, maxBuffer: MAX_EXEC_BUFFER
          }
        );
        healed.push(issues[i]);
        console.log(
          '[SkillsMonitor] Auto-healed ' + skillName + ': npm install'
        );
      } catch (e) {
        console.error(
          '[SkillsMonitor] Failed to heal ' + skillName + ': ' + e.message
        );
      }
      ...
      try {
        ...
        healed.push(issues[i]);
        console.log(
          '[SkillsMonitor] Auto-healed ' + skillName + ': created SKILL.md stub'
        );
      } catch (e) {}
    }
  }
