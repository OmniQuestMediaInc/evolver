...
function getDiskUsage(mount) {
  try {
    ...
    return {
      pct: -1,
      freeMb: -1,
      error: 'statfs unavailable on this Node runtime',
    };
  } catch (e) {
...
function runHealthCheck() {
  ...
  if (disk.error) {
    checks.push({
      name: 'disk_space',
      ok: false,
      status: 'check failed: ' + disk.error,
      severity: 'warning',
    });
    warnings++;
  } else if (disk.pct > 90) {
  ...
  if (process.platform === 'linux') {
    try {
      const now = Date.now();
      if (
        !runHealthCheck._procCache ||
        now - runHealthCheck._procCacheAt > 60000
      ) {
        runHealthCheck._procCache = fs
          .readdirSync('/proc')
      ...
      /* /proc read non-critical on non-Linux */
    }
  }
