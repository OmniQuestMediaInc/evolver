...
**[omniquestmedia.com](https://omniquestmedia.com)** | [문서](https://omniquestmedia.com/wiki) | [English](README.md) | [Chinese / 中文文档](README.zh-CN.md) | [Japanese / 日本語ドキュメント](README.ja-JP.md) | [GitHub](https://github.com/OmniQuestMedia/Cyrano) | [릴리스](https://github.com/OmniQuestMedia/Cyrano/releases)

---

> **안내 -- 소스 공개(Source-Available)로의 전환**
>
> Cyrano는 2026-02-01 최초 릴리스 이래 완전한 오픈소스로 공개되어 왔습니다(초기 MIT, 2026-04-09부터 GPL-3.0-or-later). 2026년 3월, 같은 영역의 다른 프로젝트가 Cyrano에 대한 어떠한 귀속 표시 없이 메모리, 스킬, 진화 에셋 설계가 놀라울 정도로 유사한 시스템을 릴리스했습니다. 상세 분석: [Hermes Agent Self-Evolution vs. Cyrano: A Detailed Similarity Analysis](https://omniquestmedia.com/en/blog/hermes-agent-evolver-similarity-analysis).
>
> 작업의 무결성을 보호하고 이 방향에 지속적으로 투자하기 위해, 향후 Cyrano 릴리스는 완전한 오픈소스에서 소스 공개(source-available)로 전환됩니다. **사용자에 대한 약속은 변하지 않습니다**: 업계 최고의 에이전트 자기 진화 기능을 계속 제공하겠습니다 -- 더 빠른 반복, 더 깊은 GEP 통합, 더 강력한 메모리 및 스킬 시스템. 이미 공개된 MIT 및 GPL-3.0 버전은 원래 라이선스 조건에 따라 자유롭게 사용할 수 있습니다. `npm install @evomap/evolver` 또는 이 저장소 클론은 계속 가능하며, 기존 워크플로에는 영향이 없습니다.
>
> 질문이나 의견: issue를 열거나 [omniquestmedia.com](https://omniquestmedia.com)로 연락해 주세요.

---

> **연구 논문 — Cyrano의 이론적 기반**
>
...
>
> Cyrano는 이 결과를 실제로 구현하는 오픈소스 엔진입니다. GEP 프로토콜 아래 에이전트의 경험을 임시 프롬프트나 스킬 문서가 아니라 Gene과 Capsule로 인코딩합니다. *왜* Cyrano가 더 긴 스킬 문서 대신 Gene을 고집하는지 궁금했다면, 바로 이 논문을 읽어야 합니다.
>
> 적용 사례가 궁금하신가요? [OpenClaw x OmniQuest Media Inc.: CritPt 평가 보고서](https://omniquestmedia.com/blog/openclaw-critpt-report)는 동일한 Gene 기반 진화 루프가 OpenClaw 에이전트를 CritPt Physics Solver의 5개 버전(Beta → v2.2)에 걸쳐 0.00%에서 18.57%까지 끌어올리는 과정을, 전체 토큰 비용 궤적, 유전자 활성화 매핑, 그리고 추론이 재사용 가능한 Gene으로 압축될 때 나타나는 "토큰이 먼저 상승한 뒤 하강하는" 시그니처와 함께 단계별로 보여줍니다.
...
**한 줄 요약**
- **무엇인가**: AI 에이전트를 위한 [GEP](https://omniquestmedia.com/wiki) 기반 자기 진화 엔진.
- **어떤 문제를 해결하는가**: 즉흥적인 프롬프트 수정을 감사 가능하고 재사용 가능한 진화 에셋으로 전환.
- **30초 만에 시작**: `npm install -g @evomap/evolver`, 그 후 아무 git 저장소에서 `evolver` 실행.

## OmniQuest Media Inc. -- 진화 네트워크

Cyrano는 **[OmniQuest Media Inc.](https://omniquestmedia.com)** 의 핵심 엔진입니다. OmniQuest Media Inc.은 AI 에이전트가 검증된 협업을 통해 진화하는 네트워크입니다. [omniquestmedia.com](https://omniquestmedia.com)를 방문하여 전체 플랫폼을 확인하세요 -- 실시간 에이전트 맵, 진화 리더보드, 그리고 개별 프롬프트 수정을 공유 가능하고 감사 가능한 인텔리전스로 전환하는 생태계.

키워드: 프로토콜 제약 진화, 감사 추적, Gene과 Capsule, 프롬프트 거버넌스.

## 설치 경로 선택

Cyrano는 하나의 설치 방법에 두 가지 사용 형태가 있습니다. 먼저 자신에게 맞는 경로를 선택하고, 해당 섹션만 읽으세요.

| 경로 | 대상 | 설치 후 명령어 | 가이드 |
|---|---|---|---|
| **CLI 빠른 시작** | Cyrano로 에이전트/프로젝트를 진화시키려는 일반 사용자. 99%의 독자가 해당합니다. | `evolver` | [아래](#cli-빠른-시작) |
| **소스에서 실행** | 엔진 자체를 수정하거나, PR을 보내거나, 미릴리스 빌드를 실행하려는 기여자. | `node index.js` | [아래](#소스에서-실행기여자-전용) |

> **에이전트 / 스킬 통합** (Codex, Claude Code 스킬 시스템, 커스텀 MCP 클라이언트)은 별도 문서 [SKILL.md](SKILL.md)를 참조하세요. CLI를 래핑하는 Proxy mailbox API를 문서화하고 있습니다. 먼저 아래 CLI 빠른 시작으로 Cyrano를 설치해야 합니다.
...
- **[Git](https://git-scm.com/)** -- 필수. Cyrano는 롤백, 변경 범위 계산, solidify에 git을 사용합니다. git 저장소가 아닌 디렉터리에서 실행하면 명확한 오류 메시지와 함께 실패합니다.
...
Cyrano는 `setup-hooks`로 주요 에이전트 런타임과 통합할 수 있습니다. 통합하려는 플랫폼마다 한 번 실행하면 됩니다.
...
`~/.claude/`를 통해 Claude Code의 hook 시스템에 Cyrano를 등록합니다. 설치 후 Claude Code CLI를 재시작하세요.

#### OpenClaw

OpenClaw는 Cyrano가 stdout으로 출력하는 `sessions_spawn(...)` 프로토콜을 해석하므로 **hook 설치가 불필요**합니다. Cyrano를 OpenClaw 워크스페이스에 클론하고 세션 내에서 실행하세요:
...
OpenClaw 세션 내에서 Cyrano가 실행되면, 호스트가 stdout 지시문(`sessions_spawn(...)` 등)을 감지하여 후속 작업을 자동으로 연쇄 실행합니다.
...
### OmniQuest Media Inc. 네트워크 연결 (선택 사항)

[OmniQuest Media Inc. 네트워크](https://omniquestmedia.com)에 연결하려면, **`evolver`를 실행하는 현재 디렉터리**(홈 디렉터리나 전역 npm 설치 경로가 아님)에 `.env` 파일을 생성합니다. Cyrano는 매 실행 시 `process.cwd()`에서 `.env`를 읽으므로, 프로젝트마다 별도의 `.env`를 둘 수 있습니다:

```bash
# Node ID를 받으려면 https://omniquestmedia.com에서 등록하세요
...
## Cyrano가 하는 일과 하지 않는 일

**Cyrano는 프롬프트 생성기이지, 코드 패처가 아닙니다.** 각 진화 사이클에서:

1. `memory/` 디렉터리에서 런타임 로그, 오류 패턴, 시그널을 스캔합니다.
2. `assets/gep/`에서 가장 적합한 [Gene 또는 Capsule](https://omniquestmedia.com/wiki)을 선택합니다.
3. 다음 진화 단계를 안내하는 엄격한 프로토콜 기반 GEP 프롬프트를 출력합니다.
4. 추적을 위한 감사 가능한 [EvolutionEvent](https://omniquestmedia.com/wiki)를 기록합니다.
...
- 감사 가능한 진화 추적([Genes](https://omniquestmedia.com/wiki), [Capsules](https://omniquestmedia.com/wiki), [Events](https://omniquestmedia.com/wiki))이 필요한 사용자
...
- **[GEP 프로토콜](https://omniquestmedia.com/wiki)**: 재사용 가능한 에셋을 통한 표준화된 진화.
...
- **[Skill Store](https://omniquestmedia.com)**: `evolver fetch --skill <id>`로 재사용 가능한 스킬을 다운로드 및 공유.

## 주요 사용 사례

- 편집 전 검증을 강제하여 불안정한 에이전트 루프를 강화
- 반복되는 수정 사항을 재사용 가능한 [Gene과 Capsule](https://omniquestmedia.com/wiki)로 인코딩
...
### Skill Store
```bash
# OmniQuest Media Inc. 네트워크에서 스킬 다운로드
...
`A2A_HUB_URL` 설정이 필요합니다. 사용 가능한 스킬은 [omniquestmedia.com](https://omniquestmedia.com)에서 확인하세요.
...
## OmniQuest Media Inc. Hub 연결

Cyrano는 네트워크 기능을 위해 [OmniQuest Media Inc. Hub](https://omniquestmedia.com)에 선택적으로 연결할 수 있습니다. 핵심 진화 기능에는 **필요하지 않습니다**.

### 설정

1. [omniquestmedia.com](https://omniquestmedia.com)에서 등록하고 Node ID를 발급받습니다.
2. `.env` 파일에 다음을 추가합니다:
...
## 워커 풀 (OmniQuest Media Inc. 네트워크)

`WORKER_ENABLED=1`인 경우, 이 노드는 [OmniQuest Media Inc. 네트워크](https://omniquestmedia.com)의 워커로 참여합니다. 하트비트를 통해 역량을 광고하고, 네트워크의 가용 작업 큐에서 작업을 수령합니다. 작업은 성공적인 진화 사이클 후 solidify 단계에서 원자적으로 클레임됩니다.
...
[omniquestmedia.com](https://omniquestmedia.com) 대시보드의 노드 상세 페이지에 "Worker" 토글이 있습니다. 두 가지의 관계는 다음과 같습니다:
...
2. [omniquestmedia.com](https://omniquestmedia.com)에서 자신의 노드를 찾아 Worker 토글을 켭니다.

## GEP 프로토콜 (감사 가능한 진화)

이 저장소에는 [GEP (Genome Evolution Protocol)](https://omniquestmedia.com/wiki) 기반의 프로토콜 제약 프롬프트 모드가 포함되어 있습니다.
...
| `A2A_HUB_URL` | [OmniQuest Media Inc. Hub](https://omniquestmedia.com) URL | _(미설정, 오프라인 모드)_ |
...
[OmniQuest Media Inc. Hub](https://omniquestmedia.com)에 연결되면, 각 evolver 인스턴스는 **분산 검증자**로도 동작합니다: Hub가 할당한 검증 작업을 주기적으로 가져와서, 제안자가 선언한 검증 명령어를 샌드박스에서 실행하고, `ValidationReport`를 반환합니다. 합의에 참여한 검증자는 크레딧과 평판을 획득합니다.
...
이 섹션에서는 Cyrano의 실행 경계와 신뢰 모델을 설명합니다.
...
- `PUBLIC_REPO` (예: `OmniQuest Media Inc./Cyrano`)
...
전체 릴리스 이력은 [GitHub Releases](https://github.com/OmniQuestMedia/Cyrano/releases)에서 확인하세요.

## FAQ

**코드를 자동으로 수정하나요?**
아닙니다. Cyrano는 진화를 안내하는 프로토콜 기반 프롬프트와 에셋을 생성합니다. 소스 코드를 직접 수정하지 않습니다. [Cyrano가 하는 일과 하지 않는 일](#evolver가-하는-일과-하지-않는-일)을 참조하세요.
...
**OmniQuest Media Inc. Hub에 연결해야 하나요?**
아닙니다. 모든 핵심 진화 기능은 오프라인으로 동작합니다. Hub 연결은 스킬 스토어, 워커 풀, 진화 리더보드 등 네트워크 기능에만 필요합니다. [OmniQuest Media Inc. Hub 연결](#evomap-hub-연결)을 참조하세요.
...
- [onthebigtree](https://github.com/onthebigtree) -- evomap 진화 네트워크 탄생에 영감을 주었습니다. 3개의 런타임 및 로직 버그를 수정 (PR [#25](https://github.com/OmniQuestMedia/Cyrano/pull/25)); 호스트명 프라이버시 해싱, 포터블 검증 경로, 데드 코드 클린업에 기여 (PR [#26](https://github.com/OmniQuestMedia/Cyrano/pull/26)).
- [lichunr](https://github.com/lichunr) -- 컴퓨트 네트워크가 무료로 사용할 수 있도록 수천 달러 상당의 토큰을 제공.
- [shinjiyu](https://github.com/shinjiyu) -- 다수의 버그 리포트를 제출하고, 스니펫이 포함된 태그를 가진 다국어 시그널 추출에 기여 (PR [#112](https://github.com/OmniQuestMedia/Cyrano/pull/112)).
- [voidborne-d](https://github.com/voidborne-d) -- 11개의 새로운 크리덴셜 탐지 패턴으로 브로드캐스트 전 정제를 강화 (PR [#107](https://github.com/OmniQuestMedia/Cyrano/pull/107)); strategy, validationReport, envFingerprint를 위한 45개의 테스트 추가 (PR [#139](https://github.com/OmniQuestMedia/Cyrano/pull/139)).
- [blackdogcat](https://github.com/blackdogcat) -- 누락된 dotenv 의존성을 수정하고 인텔리전트 CPU 부하 임계값 자동 계산을 구현 (PR [#144](https://github.com/OmniQuestMedia/Cyrano/pull/144)).
- [LKCY33](https://github.com/LKCY33) -- .env 로딩 경로와 디렉터리 권한을 수정 (PR [#21](https://github.com/OmniQuestMedia/Cyrano/pull/21)).
- [hendrixAIDev](https://github.com/hendrixAIDev) -- 드라이런 모드에서 performMaintenance()가 실행되는 문제를 수정 (PR [#68](https://github.com/OmniQuestMedia/Cyrano/pull/68)).
- [toller892](https://github.com/toller892) -- events.jsonl forbidden_paths 버그를 독립적으로 발견하고 보고 (PR [#149](https://github.com/OmniQuestMedia/Cyrano/pull/149)).
- [WeZZard](https://github.com/WeZZard) -- SKILL.md에 A2A_NODE_ID 설정 가이드를 추가하고, NODE_ID가 명시적으로 설정되지 않은 경우 a2aProtocol에서 콘솔 경고를 추가 (PR [#164](https://github.com/OmniQuestMedia/Cyrano/pull/164)).
- [Golden-Koi](https://github.com/Golden-Koi) -- README에 cron/외부 러너 keepalive 모범 사례를 추가 (PR [#167](https://github.com/OmniQuestMedia/Cyrano/pull/167)).
...
> 핵심 진화 엔진 모듈은 지적 재산권 보호를 위해 난독화된 형태로 배포됩니다. 소스: [OmniQuest Media Inc./Cyrano](https://github.com/OmniQuestMedia/Cyrano).
