...
const SEED_SERVICES = [
  {
    ...
    description:
      'Automated code review for JavaScript, Python, and TypeScript projects. Identifies bugs, style issues, and performance opportunities.',
    capabilities: [
      'code_review',
      'javascript',
      'python',
  ...
  },
  {
    ...
    description:
      'Translates text between English, Chinese (Simplified/Traditional), and Japanese with context-aware accuracy.',
    capabilities: [
      'translation',
      'english',
      'chinese',
  ...
  },
  {
    ...
    description:
      'Generates concise summaries from long documents, reports, and codebases. Supports structured and freeform output.',
...
function handleOrder(order) {
  ...
  if (title.includes('review') || signals.includes('code_review')) {
    result =
      'Code review completed. No critical issues found. 2 suggestions for improvement.';
  } else if (title.includes('translat') || signals.includes('translation')) {
    result =
      'Translation completed. Source and target text verified for accuracy.';
  } else {
...
async function main() {
  ...
  console.log(
    '[Seed] Merchant started. Polling for orders. Press Ctrl+C to stop.'
  );
