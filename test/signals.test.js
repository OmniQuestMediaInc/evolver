...
function hasSignal(signals, name) {
  return (
    Array.isArray(signals) && signals.some(s => String(s).startsWith(name))
  );
}
  ...
  it('does NOT detect on linux even with matching content', () => {
    ...
    try {
      const r = extractSignals({
        ...emptyInput,
        recentSessionTranscript:
          'Running pgrep -f evolver and ps aux and cat > file',
      });
      assert.ok(
        !hasSignal(r, 'windows_shell_incompatible'),
        'should not flag on linux'
      );
    } finally {
      restorePlatform();
    }
...
describe('_extractKeywordScore -- weighted keyword scoring (Layer 2)', () => {
  ...
  it('each profile has keywords and threshold', () => {
    for (const [name, profile] of Object.entries(SIGNAL_PROFILES)) {
      ...
      assert.ok(
        ...
        name + ' missing valid threshold'
      );
      assert.ok(
        Object.keys(profile.keywords).length > 0,
        name + ' has empty keywords'
      );
    }
  });
  ...
  it('detects perf_bottleneck from accumulated keywords', () => {
    const corpus =
      'the system is slow and has high latency, queries timeout frequently and the bottleneck is the database';
    const r = _extractKeywordScore(corpus.toLowerCase());
    assert.ok(
      r.includes('perf_bottleneck'),
      'expected perf_bottleneck, got ' + JSON.stringify(r)
    );
  });
  ...
  it('does not fire perf_bottleneck on a single weak keyword', () => {
    ...
    assert.ok(
      !r.includes('perf_bottleneck'),
      'single weak keyword should not fire perf_bottleneck'
    );
  });

  it('detects capability_gap from accumulated keywords', () => {
    const corpus =
      'this feature is not supported and the API is not available for this use case. it is not implemented.';
    const r = _extractKeywordScore(corpus.toLowerCase());
    assert.ok(
      r.includes('capability_gap'),
      'expected capability_gap, got ' + JSON.stringify(r)
    );
  });

  it('detects user_feature_request from accumulated keywords', () => {
    const corpus =
      'we need to implement a new feature, i want to add support for the endpoint, please add a new capability';
    const r = _extractKeywordScore(corpus.toLowerCase());
    assert.ok(
      r.includes('user_feature_request'),
      'expected user_feature_request, got ' + JSON.stringify(r)
    );
  });

  it('detects user_improvement_suggestion from keywords', () => {
    const corpus =
      'we should improve the codebase, refactor the module, and streamline the process to be more efficient';
    const r = _extractKeywordScore(corpus.toLowerCase());
    assert.ok(
      r.includes('user_improvement_suggestion'),
      'expected user_improvement_suggestion, got ' + JSON.stringify(r)
    );
  });

  it('detects recurring_error from keywords', () => {
    const corpus =
      'the same error keeps happening repeatedly, it crashed again, still failing after the fix, exception not fixed';
    const r = _extractKeywordScore(corpus.toLowerCase());
    assert.ok(
      r.includes('recurring_error'),
      'expected recurring_error, got ' + JSON.stringify(r)
    );
  });

  it('detects evolution_stagnation_detected from keywords', () => {
    const corpus =
      'no change in the output, same result every time, stuck on this plateau with no progress at all';
    const r = _extractKeywordScore(corpus.toLowerCase());
    assert.ok(
      r.includes('evolution_stagnation_detected'),
      'expected evolution_stagnation_detected, got ' + JSON.stringify(r)
    );
  });

  it('returns empty for neutral text', () => {
    const corpus =
      'the sun is shining today and the weather is nice. the cat sat on the mat.';
    const r = _extractKeywordScore(corpus.toLowerCase());
    assert.ok(
      r.length === 0,
      'neutral text should not trigger signals, got ' + JSON.stringify(r)
    );
  });

  it('can fire multiple signals from mixed corpus', () => {
    const corpus =
      'the system is slow with high latency timeout. also this feature is not supported and not implemented.';
    const r = _extractKeywordScore(corpus.toLowerCase());
...
describe('extractSignals -- multi-strategy integration', () => {
  it('keyword-scoring adds signals that regex misses', () => {
    ...
    const r = extractSignals({
      ...emptyInput,
      todayLog:
        'request had lag and delay, the system felt sluggish with throttle. performance was degraded.',
    });
    assert.ok(
      hasSignal(r, 'perf_bottleneck'),
      'keyword scoring should detect perf_bottleneck from accumulated weak keywords, got ' +
        JSON.stringify(r)
    );
  });
  ...
  it('all signals are deduplicated in final output', () => {
    ...
    for (const [key, count] of Object.entries(counts)) {
      ...
      assert.ok(
        count === 1,
        key + ' appeared ' + count + ' times (should be 1)'
      );
    }
  });
});
...
describe('signals.js source hardening (GHSA-j5w5-568x-rq53)', () => {
  it('does not use execSync with string-concatenated shell commands', () => {
    ...
    const src = fs.readFileSync(
      path.join(__dirname, '..', 'src', 'gep', 'signals.js'),
      'utf8'
    );
    ...
    assert.ok(
      !/execSync\s*\(\s*curlCmd/.test(src),
      'execSync(curlCmd) is forbidden (command injection)'
    );
    assert.ok(
      !/execSync\([^)]*['\"]curl /.test(src),
      'execSync with inline curl string is forbidden'
    );
    // execFileSync is the mandated replacement.
    assert.ok(
      /execFileSync/.test(src),
      'signals.js must use execFileSync instead of execSync for HTTP'
    );
  });
});
