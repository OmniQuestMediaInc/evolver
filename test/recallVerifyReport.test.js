...
const {
  ...
  parseSince,
  parseArgs,
} = require('../scripts/recall-verify-report');
...
describe('recall-verify-report.parseArgs', () => {
  it('parses --since, --json, --since=<value>', () => {
    ...
    assert.deepEqual(parseArgs(['n', 's', '--json']), {
      since: null,
      json: true,
    });
    assert.deepEqual(parseArgs(['n', 's', '--since', '1h']), {
      since: '1h',
      json: false,
    });
    assert.deepEqual(parseArgs(['n', 's', '--since=30m', '--json']), {
      since: '30m',
      json: true,
    });
  });
...
describe('recall-verify-report.aggregate', () => {
  ...
  it('aggregates per-type counts and excludes skipped from success_rate', () => {
    ...
    const cap = r.rows.find(function (x) {
      return x.type === 'Capsule';
    });
    ...
    const sb = r.rows.find(function (x) {
      return x.type === 'SkillBundle';
    });
  ...
  it('sets gate=YELLOW when 0.85 <= success_rate < 0.95', () => {
    const events = [];
    for (let i = 0; i < 9; i++)
      events.push(mkEvent('Capsule', 'roundtrip_ok', 100, 5000));
  ...
  it('does NOT downgrade gate from RED to YELLOW across rows', () => {
    ...
    for (let i = 0; i < 10; i++)
      events.push(mkEvent('AntiPattern', 'roundtrip_missing', 100, 5000));
    ...
    assert.equal(
      r.gate,
      'RED',
      'AntiPattern RED must not be downgraded by Capsule YELLOW'
    );
  });
  ...
  it('does NOT downgrade gate from YELLOW to GREEN across rows', () => {
    ...
    for (let i = 0; i < 10; i++)
      events.push(mkEvent('Gene', 'roundtrip_ok', 100, 5000));
    const r = aggregate(events);
    assert.equal(r.gate, 'YELLOW');
  });
...
describe('recall-verify-report CLI exit codes', () => {
  ...
  function runReport(extraArgs) {
    ...
    const cmd =
      'node ' +
      path.join(__dirname, '..', 'scripts', 'recall-verify-report.js') +
      ' ' +
      args;
    try {
...
function mkEventWithTs(
  ...
  return {
    ...
    asset: {
      type,
      id: 'sha256:dummy' + Math.random().toString(36).slice(2, 8),
    },
    verification: {
      outcome,
      reason: reason || null,
