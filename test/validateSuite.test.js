...
describe('scripts/validate-suite.js (community PR #514)', () => {
  it('source uses execFileSync and not shell-interpolated execSync', () => {
    const src = fs.readFileSync(
      path.join(REPO_ROOT, 'scripts', 'validate-suite.js'),
      'utf8'
    );
    assert.ok(
  ...
  it('accepts a single test file path as its argument', () => {
    ...
    assert.ok(
      fs.existsSync(path.join(REPO_ROOT, targetTest)),
      'probe test file must exist: ' + targetTest
    );

    const output = execFileSync(
      process.execPath,
      ['scripts/validate-suite.js', targetTest],
      {
        cwd: REPO_ROOT,
        encoding: 'utf8',
        timeout: 60_000,
    ...
    assert.ok(
      /ok: \d+ test\(s\) passed/.test(output),
