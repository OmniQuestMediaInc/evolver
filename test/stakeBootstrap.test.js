...
describe('stakeBootstrap retry state machine', function () {
  ...
  beforeEach(() => {
    ...
    restoreA2a = installA2aProtocolStub(
      'node-test-stake',
  ...
  it('success resets backoff and schedules next attempt ~24h away', async () => {
    ...
    assert.ok(
      state.nextAttemptAt > Date.now() + 23 * 60 * 60 * 1000,
      'next attempt should be ~24h from now'
    );
  });
  ...
  it('network error increments transient failures and schedules first backoff (5min)', async () => {
    ...
    assert.ok(
      delay >= 4 * 60 * 1000 && delay <= 6 * 60 * 1000,
      `first transient delay should be ~5min, got ${delay}`
    );
  });
  ...
  it('402 insufficient_credits classified as funds and uses funds backoff (~60min first)', async () => {
    ...
    restoreFetch = stubFetch(async () => ({
      ...
      status: 402,
      text: async () =>
        JSON.stringify({ error: 'insufficient_credits: need 100, have 42' }),
    }));
    ...
    const delay = state.nextAttemptAt - Date.now();
    assert.ok(
      delay >= 59 * 60 * 1000 && delay <= 61 * 60 * 1000,
      `first funds delay should be ~60min, got ${delay}`
    );
  });
  ...
  it('400 stake_amount_must_be_at_least_100 classified as permanent (disabled_until_restart)', async () => {
    ...
    restoreFetch = stubFetch(async () => ({
      ...
      status: 400,
      text: async () =>
        JSON.stringify({ error: 'stake_amount_must_be_at_least_100' }),
    }));
  ...
  it('backoff skip short-circuits until nextAttemptAt, then allows retry after force', async () => {
    ...
    const delay = state.nextAttemptAt - Date.now();
    assert.ok(
      delay >= 14 * 60 * 1000 && delay <= 16 * 60 * 1000,
      `second transient delay should be ~15min, got ${delay}`
    );
  });
  ...
  it('success persists nextAttemptAt to disk under EVOLVER_HOME', async () => {
    ...
    assert.ok(
      fs.existsSync(sb.STATE_FILE),
      'state file should exist after success'
    );
    const raw = JSON.parse(fs.readFileSync(sb.STATE_FILE, 'utf8'));
    assert.ok(
      raw.nextAttemptAt > Date.now() + 23 * 60 * 60 * 1000,
      'persisted nextAttemptAt should be ~24h out'
    );
  ...
  it('persisted backoff survives a simulated process restart (module re-require)', async () => {
    ...
    assert.ok(
      persistedNext > Date.now(),
      'pre-restart state should have a future nextAttemptAt'
    );
    restoreFetch();
    restoreFetch = null;
    ...
    assert.equal(
      networkCalls,
      0,
      'must not hit hub while within persisted backoff window'
    );
    const state = sb2._getStateForTests();
    assert.equal(
      state.transientFailures,
      1,
      'failure counter should be loaded from disk'
    );
    // nextAttemptAt round-trip should be within 1ms of original.
    assert.ok(
      Math.abs(state.nextAttemptAt - persistedNext) < 2,
      `nextAttemptAt should round-trip, got ${state.nextAttemptAt} vs ${persistedNext}`
    );
  });
    ...
    restoreFetch = stubFetch(async () => {
      ...
      return {
        ...
        text: async () =>
          JSON.stringify({
            status: 'ok',
            stake: { stake_amount: 100, status: 'active' },
          }),
      };
    });
    const sb2 = freshRequire('../src/gep/validator/stakeBootstrap');
  ...
  it('load clamps absurdly-far nextAttemptAt (clock-skew or corrupt file)', async () => {
    ...
    assert.ok(
      loaded.nextAttemptAt <= Date.now() + 24 * 60 * 60 * 1000 + 1000,
