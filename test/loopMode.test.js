...
describe('loop-mode auto reject', () => {
  it('marks pending runs rejected without deleting untracked files', () => {
    ...
    const changed = rejectPendingRun(
      path.join(stateDir, 'evolution_solidify_state.json')
    );
    ...
    assert.equal(
      state.last_solidify.reason,
      'loop_bridge_disabled_autoreject_no_rollback'
    );
    assert.equal(
      fs.readFileSync(path.join(tmpDir, 'PR_BODY.md'), 'utf8'),
      'keep me\n'
    );
  });
});
...
describe('loop-mode non-fatal error handling', () => {
  ...
  it('loop-mode continues after evolve.run() throws', () => {
    ...
    assert.ok(
      ...
      'loop-mode should exit cleanly, got code: ' +
        exitCode +
        ', stdout: ' +
        stdout.slice(0, 200)
    );
  ...
  it('should_explore branch does not leak errors to cycle loop', async () => {
    ...
    assert.ok(
      !stdout.includes('TypeError: Cannot') &&
        !stdout.includes('Error: ENOENT'),
      'explore branch should not leak filesystem errors: ' +
        stdout.slice(0, 300)
    );
  });
});
...
describe('bare invocation routing -- black-box', () => {
  ...
  it('node index.js (no args) starts evolution, not help', () => {
    ...
    assert.ok(
      ...
      'bare invocation should start evolution, not show usage. Got: ' +
        out.slice(0, 200)
    );
    assert.ok(
      !out.includes('Usage:'),
      'should not show usage for bare invocation'
    );
  });
