...
describe('validateSynthesizedGene', () => {
  ...
  it('rejects gene with truthy but invalid category', () => {
    ...
    assert.ok(
      !result.valid,
      'expected invalid result, got: ' + JSON.stringify(result)
    );
    assert.ok(
      result.errors.some(function (e) {
        return e.includes('invalid category') && e.includes('deploy');
      }),
...
describe('analyzePatterns', () => {
  ...
  it('identifies high-frequency groups (count >= 5)', () => {
    ...
    for (var i = 0; i < 6; i++) {
      caps.push(
        makeCapsule('c' + i, 'gene_a', 'success', 0.9, ['error', 'crash'])
      );
    }
    writeCapsules(caps);
    var data = collectDistillationData();
...
describe('buildDistillationPrompt', () => {
  ...
  it('does not mention npm or npx as allowed validation prefixes', () => {
    ...
    assert.ok(
      !prompt.includes('"npm "'),
      'prompt must not suggest npm as valid prefix'
    );
    assert.ok(
      !prompt.includes('"npx "'),
      'prompt must not suggest npx as valid prefix'
    );
    assert.ok(
      prompt.includes('"node "'),
      'prompt must still require node prefix'
    );
  });
});
