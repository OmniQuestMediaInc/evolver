...
assert.ok(
  redactString('xoxb-1234567890-abcdefghij').includes(REDACTED),
  'xoxb- Slack bot token should be redacted'
);
assert.ok(
  redactString('xoxp-1234567890-abcdefghij').includes(REDACTED),
  'xoxp- Slack user token should be redacted'
);
assert.ok(
  redactString('xoxa-2-abc-def-ghi-j1234567').includes(REDACTED),
  'xoxa- Slack app token should be redacted'
);

// JSON Web Tokens (3 base64url segments)
var jwtSample =
  'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIn0.abcdefghijklmnopqrstuvwxyz12';
assert.ok(redactString(jwtSample).includes(REDACTED), 'JWT should be redacted');

// Azure storage AccountKey
assert.ok(
  redactString('AccountKey=AbCdEfGh1234567890+/==').includes(REDACTED),
  'Azure AccountKey should be redacted'
);

// Azure AD client_secret
assert.ok(
  redactString('client_secret=aB3~cD4.eF5_gH6-iJ7').includes(REDACTED),
...
// value end-to-end.
var discordSampleParts = [
  'MTEzMjI3NDU2Nzg5MDEyMzQ1',
  'ABC123',
  'qwerty_zxcv_qwertyu1234567890',
];
var discordSample = discordSampleParts.join('.');
assert.ok(
  redactString(discordSample).includes(REDACTED),
