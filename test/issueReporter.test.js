...
(async function run() {
  ...
  await withFetchMock(
    async function (url, opts) {
      assert.ok(
        ...
        'should hit search API'
      );
      assert.ok(
        String(url).includes('is%3Aopen'),
        'should request open issues'
      );
      return jsonResponse({
        items: [
          {
            number: 397,
            html_url: 'https://github.com/x/y/issues/397',
            title:
              '[Auto] Recurring failure: Repeated failures with gene: gene_gep_repair_from_errors',
            state: 'open',
          },
        ],
      });
    ...
    async function () {
      ...
      assert.ok(
        result && result.number === 397,
        'should find existing open issue'
      );
    }
  );
  ...
  await withFetchMock(
    ...
    async function () {
      const result = await findExistingIssue(
        'x/y',
        '[Auto] Recurring failure: Something new',
        'faketoken'
      );
        ...
        items: [
          {
            ...
            title: '[Auto] Recurring failure: foo',
            state: 'closed',
          },
        ],
      });
  ...
  await withFetchMock(
    ...
    async function () {
      ...
      assert.strictEqual(
        called,
        false,
        'fetch should not be called for empty title'
      );
    }
  );
  ...
  try {
    ...
    assert.ok(
      cfg,
      'getConfig should return an object when auto-issue is enabled by default'
    );
    assert.strictEqual(
      cfg.repo,
      SELF_PR_REPO,
      'issueReporter default repo must equal config.SELF_PR_REPO'
    );
    assert.strictEqual(
      SELF_PR_REPO,
      'EvoMap/Evolver',
      'SELF_PR_REPO default should be EvoMap/Evolver'
    );
  } finally {
  ...
  const k3 = computeErrorKey([
    'recurring_errsig(3x):timeout on API',
    'failure_loop_detected',
  ]);
  const k4 = computeErrorKey([
    'recurring_errsig(4x):timeout on API',
    'failure_loop_detected',
  ]);
  const k5 = computeErrorKey([
    'recurring_errsig(5x):timeout on API',
    'failure_loop_detected',
  ]);
  ...
  assert.strictEqual(
    shouldReport(
      ['failure_loop_detected', 'recurring_errsig(2x):foo'],
      gatedCfg
    ),
    false,
  ...
  await withFetchMock(
        ...
        items: [
          {
            number: 12345,
            state: 'open',
            html_url: 'https://x/y/12345',
            title:
              'META discussion about [Auto] Recurring failure: boom with unrelated context',
          },
        ],
      });
    },
    async function () {
      ...
      assert.strictEqual(
        result,
        null,
        'substring fallback must not match unrelated titles'
      );
    }
  );
