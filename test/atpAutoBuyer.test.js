...
describe('autoBuyer.considerOrder: guards and budget caps', () => {
  ...
  it('clamps budget to perOrderCap and daily remaining', async () => {
    ...
    assert.ok(
      seen[0].budget <= 10,
      'budget should be clamped to cold-start perOrderCap (10), got ' +
        seen[0].budget
    );
  ...
  it('respects daily cap across multiple calls', async () => {
    ...
    const r3 = await autoBuyer.considerOrder({
      capabilities: ['c3'],
      question: 'q3',
      budget: 100,
    });
    ...
    const spent = [r1, r2, r3]
      .filter(r => r.ok)
      .reduce((a, r) => a + (r.spent || 0), 0);
    assert.ok(
      spent <= 5,
      'spent ' + spent + ' must not exceed cold-start dailyCap (5)'
    );
    const lastHitCap = [r1, r2, r3].some(r => r.reason === 'daily_cap_reached');
    assert.ok(
      lastHitCap,
      'at least one call must be rejected with daily_cap_reached'
    );
  });
});
...
describe('autoBuyer: ledger persistence', () => {
  it('persists spend to ledger file and reloads on restart', async () => {
    ...
    hubClient.placeOrder = async () => ({
      ok: true,
      data: { order_id: 'persist_test' },
    });
    await autoBuyer.considerOrder({
      capabilities: ['x'],
      question: 'persist q',
    });
    ...
    assert.ok(
      Object.keys(parsed.dedup).length === 1,
      'dedup should have exactly one entry'
    );
  });
});
...
describe('autoBuyer: timeout protection', () => {
  it('resolves with error when hub placeOrder hangs past timeoutMs (min 500ms clamp)', async () => {
    ...
    assert.ok(
      elapsed < 1500,
      'should time out within ~500ms clamp, elapsed=' + elapsed
    );
    assert.ok(
      elapsed >= 400,
      'should not short-circuit before clamp, elapsed=' + elapsed
    );
  });
});
...
describe('autoBuyer.__internals.questionHash', () => {
  it('is stable regardless of capability array order', () => {
    const h1 = autoBuyer.__internals.questionHash({
      capabilities: ['a', 'b'],
      question: 'same',
    });
    const h2 = autoBuyer.__internals.questionHash({
      capabilities: ['b', 'a'],
      question: 'same',
    });
    assert.equal(h1, h2);
