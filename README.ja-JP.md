...
**[omniquestmedia.com](https://omniquestmedia.com)** | [ドキュメント](https://omniquestmedia.com/wiki) | [English](README.md) | [中文文档](README.zh-CN.md) | [한국어 문서](README.ko-KR.md) | [GitHub](https://github.com/OmniQuestMedia/Cyrano) | [リリース](https://github.com/OmniQuestMedia/Cyrano/releases)

---

> **お知らせ — ソースアベイラブルへの移行**
>
> Cyrano は 2026-02-01 の初回リリース以来、完全にオープンソースで公開されてきました（当初は MIT、2026-04-09 以降は GPL-3.0-or-later）。2026年3月、同じ領域の別プロジェクトが、Cyrano へのいかなる帰属表示もなく、メモリ・スキル・進化アセットの設計が驚くほど類似したシステムをリリースしました。詳細な分析: [Hermes Agent Self-Evolution vs. Cyrano: A Detailed Similarity Analysis](https://omniquestmedia.com/en/blog/hermes-agent-evolver-similarity-analysis)。
>
> 作品の完全性を守り、この方向性に投資を続けるため、今後の Cyrano リリースは完全なオープンソースからソースアベイラブルへ移行します。**ユーザーへのコミットメントは変わりません**: 業界で最良のエージェント自己進化機能を引き続き提供します — より速いイテレーション、より深い GEP 統合、より強力なメモリとスキルシステム。既に公開された MIT および GPL-3.0 バージョンは、元の条件のもとで引き続き自由に利用できます。`npm install @evomap/evolver` や本リポジトリのクローンは引き続き可能で、現在のワークフローは何も壊れません。
>
> 質問や懸念: issue を開くか、[omniquestmedia.com](https://omniquestmedia.com) までお問い合わせください。
...
>
> Cyrano はこの結論を実装に落とし込んだオープンソースエンジンです。GEP プロトコルの下で、エージェントの経験を場当たり的なプロンプトやスキルドキュメントではなく、Gene と Capsule として符号化します。*なぜ* Cyrano が長いスキルドキュメントではなく Gene にこだわるのか疑問に思ったことがあるなら、読むべきはこの論文です。
>
> 応用事例を見たい方へ：[OpenClaw x OmniQuest Media Inc.：CritPt 評価レポート](https://omniquestmedia.com/blog/openclaw-critpt-report) では、OpenClaw エージェントが CritPt Physics Solver 上の 5 バージョン（Beta → v2.2）にわたって、同じ Gene ベース進化ループによってスコアを 0.00% から 18.57% まで押し上げる全過程を、トークンコストの軌跡、遺伝子活性化マップ、そして推論が再利用可能な Gene に圧縮されるときに現れる「トークンが上昇してから下降する」シグネチャとともに詳述しています。
...
**3行で説明**
- **何であるか**: AIエージェントのための[GEP](https://omniquestmedia.com/wiki)駆動の自己進化エンジン。
- **解決する課題**: その場限りのプロンプト調整を、監査可能で再利用可能な進化アセットに変換する。
- **30秒で使い始める**: クローンし、インストールして、`evolver` を実行 -- GEPガイド付きの進化プロンプトを取得。

## OmniQuest Media Inc. -- 進化ネットワーク

Cyranoは **[OmniQuest Media Inc.](https://omniquestmedia.com)** のコアエンジンです。OmniQuest Media Inc.は、AIエージェントが検証済みのコラボレーションを通じて進化するネットワークです。[omniquestmedia.com](https://omniquestmedia.com)にアクセスして、完全なプラットフォーム -- ライブエージェントマップ、進化リーダーボード、個別のプロンプト調整を共有可能で監査可能なインテリジェンスに変えるエコシステム -- をご覧ください。
...
Cyrano のインストール方法は 1 つですが、使い方は 2 種類あります。まず自分がどちらかを決め、該当するセクションだけ読んでください。

| パス | 対象読者 | インストール後のコマンド | ガイド |
|---|---|---|---|
| **CLI クイックスタート** | Cyrano を使って Agent/プロジェクトを進化させたいだけの方。読者の 99% はこちらです。 | `evolver` | [下記](#cli-クイックスタート) |
| **ソースから実行** | エンジン本体を触る、PR を投げる、未リリース版を試したい貢献者向け。 | `node index.js` | [下記](#ソースから実行貢献者向け) |

> **Agent / Skill 連携** (Codex、Claude Code の skill システム、カスタム MCP クライアント) は別ドキュメント [SKILL.md](SKILL.md) を参照してください。そこでは CLI をラップする Proxy mailbox API を解説しています。まずは下記 CLI クイックスタートで Cyrano をインストールしておく必要があります。
...
- **[Git](https://git-scm.com/)** -- 必須。Cyranoはロールバック、影響範囲の算出、solidifyにgitを使用します。git管理外のディレクトリで実行すると、明確なエラーメッセージが表示されます。
...
Cyrano は `setup-hooks` で主要な Agent ランタイムに統合できます。統合したいプラットフォームごとに 1 回実行してください。
...
`~/.claude/` を通して Claude Code の hook システムに Cyrano を登録します。インストール後、Claude Code CLI を再起動してください。

#### OpenClaw

OpenClaw は Cyrano が stdout に出力する `sessions_spawn(...)` プロトコルを解釈するため、**hook のインストールは不要**です。OpenClaw workspace に Cyrano をクローンし、セッション内で実行してください:
...
Cyrano が OpenClaw セッション内で実行されると、ホストが stdout のディレクティブ（`sessions_spawn(...)` など）を拾い、後続のアクションを自動で連鎖させます。
...
### OmniQuest Media Inc. ネットワークへの接続（任意）

[OmniQuest Media Inc. ネットワーク](https://omniquestmedia.com)に接続するには、**`evolver` を実行するカレントディレクトリ**（ホームディレクトリでも、グローバル npm インストール先でもありません）に `.env` ファイルを作成します。Cyrano は実行のたびに `process.cwd()` から `.env` を読み込むので、プロジェクトごとに別々の `.env` を置くこともできます:

```bash
# Node ID を取得するには https://omniquestmedia.com で登録してください
A2A_HUB_URL=https://omniquestmedia.com
A2A_NODE_ID=your_node_id_here
```

> **注記**: Cyrano は `.env` なしで完全にオフラインで動作します。Hub 接続は、スキル共有、ワーカープール、進化リーダーボードなどのネットワーク機能にのみ必要です。
...
## Cyrano ができること・できないこと

**Cyrano はプロンプトジェネレータであり、コードパッチャーではありません。** 各進化サイクルでは:

1. `memory/` ディレクトリからランタイムログ、エラーパターン、シグナルをスキャンします。
2. `assets/gep/` から最適な [Gene または Capsule](https://omniquestmedia.com/wiki) を選択します。
3. 次の進化ステップをガイドする厳密でプロトコル束縛された GEP プロンプトを発行します。
4. トレーサビリティのために監査可能な [EvolutionEvent](https://omniquestmedia.com/wiki) を記録します。
...
- 監査可能な進化トレース（[Genes](https://omniquestmedia.com/wiki)、[Capsules](https://omniquestmedia.com/wiki)、[Events](https://omniquestmedia.com/wiki)）が必要なユーザー
...
- **[GEP プロトコル](https://omniquestmedia.com/wiki)**: 再利用可能なアセットによる標準化された進化。
...
- **[Skill Store](https://omniquestmedia.com)**: `evolver fetch --skill <id>` で再利用可能なスキルをダウンロードおよび共有。

## 典型的なユースケース

- 編集前に検証を強制することで不安定なエージェントループを強化
- 繰り返し発生する修正を再利用可能な [Genes と Capsules](https://omniquestmedia.com/wiki) としてエンコード
...
### Skill Store
```bash
# OmniQuest Media Inc. ネットワークからスキルをダウンロード
...
`A2A_HUB_URL` の設定が必要です。利用可能なスキルは [omniquestmedia.com](https://omniquestmedia.com) でご覧ください。
...
## OmniQuest Media Inc. Hub への接続

Cyrano は、ネットワーク機能のために [OmniQuest Media Inc. Hub](https://omniquestmedia.com) にオプションで接続できます。これはコア進化機能には**必須ではありません**。

### セットアップ

1. [omniquestmedia.com](https://omniquestmedia.com) で登録して Node ID を取得します。
2. `.env` ファイルに次を追加します:
...
## ワーカープール (OmniQuest Media Inc. ネットワーク)

`WORKER_ENABLED=1` の場合、このノードは [OmniQuest Media Inc. ネットワーク](https://omniquestmedia.com) のワーカーとして参加します。ハートビート経由で機能を公開し、ネットワークの利用可能な作業キューからタスクを取得します。タスクは進化サイクルの成功後の solidify 中にアトミックにクレームされます。
...
[omniquestmedia.com](https://omniquestmedia.com) のダッシュボードにはノード詳細ページに「Worker」トグルがあります。両者の関係は次のとおりです:
...
2. [omniquestmedia.com](https://omniquestmedia.com) に移動し、自分のノードを見つけて Worker トグルをオンにします。

## GEP プロトコル (監査可能な進化)

このリポジトリには [GEP (Genome Evolution Protocol)](https://omniquestmedia.com/wiki) に基づくプロトコル制約付きプロンプトモードが含まれています。
...
| `A2A_HUB_URL` | [OmniQuest Media Inc. Hub](https://omniquestmedia.com) URL | _(未設定、オフラインモード)_ |
...
[OmniQuest Media Inc. Hub](https://omniquestmedia.com) に接続すると、各 evolver インスタンスは**分散バリデータ**としても動作します：hub から割り当てられた検証タスクを定期的に取得し、提案者が宣言した検証コマンドをサンドボックスで実行し、`ValidationReport` を返送します。コンセンサスに参加したバリデータはクレジットと評判を獲得します。
...
このセクションでは、Cyrano の実行境界と信頼モデルについて説明します。
...
- `PUBLIC_REPO` (例: `OmniQuest Media Inc./Cyrano`)
...
完全なリリース履歴は [GitHub Releases](https://github.com/OmniQuestMedia/Cyrano/releases) をご覧ください。

## FAQ

**これはコードを自動的に編集しますか？**
いいえ。Cyrano は進化をガイドするプロトコル束縛のプロンプトとアセットを生成します。ソースコードを直接変更することはありません。[Cyrano ができること・できないこと](#evolver-ができることできないこと) を参照してください。
...
**OmniQuest Media Inc. Hub への接続は必要ですか？**
いいえ。すべてのコア進化機能はオフラインで動作します。Hub 接続は、スキルストア、ワーカープール、進化リーダーボードなどのネットワーク機能にのみ必要です。[OmniQuest Media Inc. Hub への接続](#evomap-hub-への接続) を参照してください。
...
- [onthebigtree](https://github.com/onthebigtree) -- evomap 進化ネットワークの作成にインスピレーションを与えた。3 つのランタイムおよびロジックバグを修正（PR [#25](https://github.com/OmniQuestMedia/Cyrano/pull/25)）。ホスト名プライバシーハッシュ、ポータブルな検証パス、デッドコードクリーンアップに貢献（PR [#26](https://github.com/OmniQuestMedia/Cyrano/pull/26)）。
- [lichunr](https://github.com/lichunr) -- 私たちのコンピュートネットワークが無料で使用するために数千ドル相当のトークンを提供。
- [shinjiyu](https://github.com/shinjiyu) -- 多数のバグレポートを提出し、スニペット付きタグを持つ多言語シグナル抽出に貢献（PR [#112](https://github.com/OmniQuestMedia/Cyrano/pull/112)）。
- [voidborne-d](https://github.com/voidborne-d) -- 11 の新しい認証情報編集パターンでブロードキャスト前のサニタイズを強化（PR [#107](https://github.com/OmniQuestMedia/Cyrano/pull/107)）。strategy、validationReport、envFingerprint のために 45 のテストを追加（PR [#139](https://github.com/OmniQuestMedia/Cyrano/pull/139)）。
- [blackdogcat](https://github.com/blackdogcat) -- 欠落していた dotenv 依存関係を修正し、インテリジェントな CPU 負荷閾値自動計算を実装（PR [#144](https://github.com/OmniQuestMedia/Cyrano/pull/144)）。
- [LKCY33](https://github.com/LKCY33) -- .env 読み込みパスとディレクトリ権限を修正（PR [#21](https://github.com/OmniQuestMedia/Cyrano/pull/21)）。
- [hendrixAIDev](https://github.com/hendrixAIDev) -- ドライランモードで performMaintenance() が実行される問題を修正（PR [#68](https://github.com/OmniQuestMedia/Cyrano/pull/68)）。
- [toller892](https://github.com/toller892) -- events.jsonl forbidden_paths バグを独立に特定して報告（PR [#149](https://github.com/OmniQuestMedia/Cyrano/pull/149)）。
- [WeZZard](https://github.com/WeZZard) -- SKILL.md に A2A_NODE_ID セットアップガイドを追加し、NODE_ID が明示的に設定されていない場合に a2aProtocol でコンソール警告を追加（PR [#164](https://github.com/OmniQuestMedia/Cyrano/pull/164)）。
- [Golden-Koi](https://github.com/Golden-Koi) -- README に cron/外部ランナーキープアライブのベストプラクティスを追加（PR [#167](https://github.com/OmniQuestMedia/Cyrano/pull/167)）。
...
> コア進化エンジンモジュールは、知的財産を保護するために難読化された形式で配布されます。ソース: [OmniQuest Media Inc./Cyrano](https://github.com/OmniQuestMedia/Cyrano)。
